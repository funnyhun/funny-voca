...
## 3. 종속성 및 연관 관계
- **migration.js (`@/common/api/user/migration.js`)**: 로그인 성공 직후 로컬 데이터를 서버로 옮기기 위해 `loadUserData.js`에서 의존.
- **master.js (`@/common/api/common/master.js`)**: 마스터 단어 및 메타데이터 정보 조회.
- **storage.js (`@/common/api/util/storage.js`)**: 로컬 스토리지에 데이터를 읽고 쓰는 데 사용됨.
- **utils.js (`@/common/utils/utils.js`)**: 로컬 상태 파싱 및 헬퍼 로직용.

## 4. 데이터 로드 및 병합 로직 (Robust Data Loading)
`loadUserData.js`는 사용자 데이터의 무결성을 위해 다음 과정을 거칩니다.

### 4.1 로그인 유저 (Member)
1. **템플릿 확보**: 로컬스토리지의 `wordMap`을 기반 구조(Template)로 사용합니다. 로컬 데이터가 없을 경우 DB(`Word` 테이블)로부터 전체 구조를 가져옵니다.
