# Agent Context Anchor - src/ui/services/Voca

본 문서는 `src/ui/services/Voca` 도메인의 컴포넌트 및 스타일 코드에 대한 에이전트 전용 컨텍스트 앵커입니다. 에이전트는 이 디렉토리 내의 파일들을 생성, 수정, 리팩토링할 때 아래의 규칙을 반드시 준수해야 합니다.
...
1. **상대 참조 금지 (상위 디렉토리 참조 시 절대 경로 사용)**
   ...
   - 반드시 **절대 경로 별칭 (`@/ui/common/...`, `@/common/api/...`, `@/ui/services/Voca/...`)**을 사용하여 모듈 참조 오류를 차단하십시오.
2. **스타일 파일의 단방향 의존성**
...
3. **진입점 최소화 (배럴 패턴)**
   - 도메인 최상위 `index.js`에 명시적으로 노출된 컴포넌트만 외부에서 사용할 수 있습니다. 외부 폴더에서 이 도메인의 서브컴포넌트를 임포트할 때는 반드시 `import { Voca, WordList } from "@/ui/services/Voca"` 형태로만 임포트해야 합니다.
